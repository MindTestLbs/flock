	...
	$(function(){
		...
		function sideBarHeight(){
			...
			var wionHeight = $(window).height(); 
			if(docHeight > wionHeight)
			{
				if(winwidth > 750)
				{	
					$(".flock-sidebar").css('min-height',docHeight );
					$(".commen-base").css('min-height',docHeight );
				}else
				{
		...
		if($('.code-popup').exists())
		{
			$(".code-popup").fancybox({
				...
				padding:0,
				tpl         : {
					wrap     : '<div class="fancybox-wrap cms-fancy-outer" tabIndex="-1"><div class="fancybox-skin"><div class="fancybox-outer"><div class="fancybox-inner"></div></div></div></div>',
				}
			});
		}
		if($('.cm-file-upload').exists())
		{
			$(".cm-file-upload").uniform({
				 fileButtonHtml:'Browse'
			 });
		}
		if($('.temp-popup').exists())
		{
			$(".temp-popup").fancybox({
				maxWidth	: '90%',
				maxHeight	: '90%',
